...
export const Home: React.FC = () => {
  ...
  const processChartData = (variable: keyof SensorData, color: string) => {
    ...
    const labels = (sensorData || []).map(item => {
      if (typeof item.timestamp === 'string') {
        return new Date(item.timestamp).toLocaleTimeString().slice(0, -3); // Remove seconds
      }
      return ''; // Default value if timestamp is not a string
    });
  ...
  return (
    <div className="relative min-h-screen pt-12">
      ...
      <div className="pt-16 px-4 flex h-screen">
        ...
        <div className="w-4/5 grid grid-cols-2 grid-rows-2 gap-4 p-4">
